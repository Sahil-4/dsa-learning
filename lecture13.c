...
void p2() {
    ...
    // Logic : using binary search 
    // take low = 0, high = <last_index>, mid = low + (high-low)/2
    // run a while loop till low < high
...
void p3() {
    ...
    // calculate right_sum by adding all elements arr[i] using for (i = 1; to i < size;)
    // run for loop (i = 0; to i < size;)
    // check if right_sum == left_sum return i as pivot index 
    // else perform below operations
    // left_sum += arr[i];
    // right_sum -= arr[i+1];
